...
shww_by_month = monthly_means['values'].values



num_forecast_steps = 12 * 5 # Forecast the final 5 years, given previous data
shww_by_month_training_data = shww_by_month[:-num_forecast_steps]

shww_dates = np.arange("1940-01", "1969-01", dtype="datetime64[M]")
shww_loc = mdates.YearLocator(3)
shww_fmt = mdates.DateFormatter('%Y')



model = build_model(shww_by_month)
...
elbo_loss_curve = tfp.vi.fit_surrogate_posterior(
    target_log_prob_fn=model.joint_distribution(
        observed_time_series=shww_by_month).log_prob,
...
shww_forecast_dist = tfp.sts.forecast(
    model,
    observed_time_series=shww_by_month,
...
fig, ax = plot_forecast(
    shww_dates, shww_by_month[0:len(shww_dates)],
